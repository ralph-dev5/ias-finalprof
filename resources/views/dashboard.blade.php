    ...
    <div class="vault-shell">
        ...
        <header class="dash-header">
            <div class="dash-header-left">
                ...
                <p class="dash-subtitle">All files encrypted before storage · SHA-256 verified on download</p>
            </div>
            <div class="dash-user">
        ...
        {{-- Upload section --}}
        <section class="upload-section" style="animation-delay: 0.05s">
            ...
            <form method="POST" action="{{ route('upload') }}" enctype="multipart/form-data" class="upload-form">
                ...
                <label for="file" class="dropzone" id="dropzone-label">
                    <div class="dropzone-inner">
                        ...
                        <p class="dropzone-text">Click to browse or drag a file here</p>
        ...
        {{-- Files section --}}
        <section class="files-section" style="animation-delay: 0.12s">
            <div class="section-label">
...
.vault-shell { position: relative; z-index: 1; max-width: 1100px; margin: 0 auto; padding: 2.5rem 1.5rem 4rem; display: flex; flex-direction: column; gap: 1.5rem; }
...
.dash-header-left { display: flex; flex-direction: column; gap: 0.4rem; }
.dash-badge {
...
.dash-badge-dot {
    ...
    box-shadow: 0 0 6px var(--amber); animation: pulse 2s infinite;
}
@keyframes pulse { 0%,100%{opacity:1} 50%{opacity:0.4} }
.dash-title { font-size: 1.6rem; font-weight: 700; color: #fff; letter-spacing: -0.02em; }
.dash-subtitle { font-size: 0.78rem; color: var(--muted); }
.dash-user {
...
.user-icon { width: 14px; height: 14px; color: var(--amber); }
...
.section-icon { width: 13px; height: 13px; color: var(--amber); }
...
.dropzone {
    ...
    border: 2px dashed var(--border-hi); border-radius: 1rem;
    padding: 2rem; cursor: pointer;
    transition: border-color 0.2s, background 0.2s;
    position: relative;
}
.dropzone:hover { border-color: rgba(245,158,11,0.4); background: var(--amber-dim); }
.dropzone-inner { display: flex; flex-direction: column; align-items: center; gap: 0.5rem; pointer-events: none; }
.dropzone-icon { width: 40px; height: 40px; color: var(--muted); }
.dropzone-text { font-size: 0.85rem; color: var(--text); font-weight: 600; }
.dropzone-hint { font-family: var(--mono); font-size: 0.65rem; color: var(--muted); letter-spacing: 0.04em; }
.file-input { position: absolute; inset: 0; opacity: 0; cursor: pointer; width: 100%; height: 100%; }

.btn { display: inline-flex; align-items: center; gap: 0.4rem; padding: 0.65rem 1.25rem; border-radius: 0.5rem; font-family: var(--sans); font-size: 0.875rem; font-weight: 600; text-decoration: none; transition: all 0.15s; border: 1px solid transparent; cursor: pointer; }
...
/* Table */
.table-shell { border-radius: 0.875rem; border: 1px solid var(--border); overflow: hidden; overflow-x: auto; }
...
.hash-text {
    ...
    font-family: var(--mono); font-size: 0.68rem; color: #9ca3af; letter-spacing: 0.02em;
}
.btn-download {
    display: inline-flex; align-items: center; gap: 0.35rem;
    padding: 0.4rem 0.85rem; border-radius: 0.4rem;
    background: var(--surface-2); border: 1px solid var(--border);
    font-size: 0.75rem; font-family: var(--sans); font-weight: 600;
...
/* Empty */
.empty-state { display: flex; flex-direction: column; align-items: center; padding: 3rem 1rem; gap: 0.6rem; text-align: center; }
.empty-icon { width: 48px; height: 48px; color: var(--border-hi); }
